...
export class TeamListComponent implements OnInit {
  displayedColumns: string[] = ['id', 'photo', 'firstname', 'lastname', 'title', 'description', 'actions'];
  ...
  onDelete(id) {
    this.swalService.confirm('Atención', 'Estás por eliminar un equipo', 'warning', true).then((result) => {
      if (result.value) {
        const url = urljoin(environment.apiUrl, `team/${id.toString()}`);
        this.teamsService.delete(url).subscribe((resp: any) => {
          this.onLoadPage();
          this.teamsService.sendTeam(null);
